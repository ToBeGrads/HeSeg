...
interface SidebarItemProps {
  structureId: number
  onAddCoordinate: () => void  // Keep this - needs parent to start placement mode
}
...
function SidebarItem({ structureId, onAddCoordinate }: SidebarItemProps) {
  ...
  const handleStartEditing = () => {
    if (isEditing) return
    console.log(`✏️ Start editing structure ${structureId}`)
      ...
      {isExpanded && (
        <div className="coordinates-list">
          <div className="coordinates-header">
            <span>Coordinates ({coordinates.length})</span>
          ...
          {coordinates.length > 0 ? (
            ...
            <div className="no-coordinates">No coordinates added yet</div>
          )}
        </div>
      )}
    </div>
